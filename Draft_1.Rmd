...
test.final <- update(step.method,.~.-income:lab.force-income:hh.size-income:lab.force)
...
The final model is $log(y_{ijkwz})=\beta_{0}+\beta_{1}x_{ijkwz}+\beta_{2}x_{ijkwz}^{2}+\alpha_{j}^{G}+\alpha_k^{S}+\alpha_{w}^{T}+\alpha_{z}^{L}+\beta^{L}x_{ijkwz}+\beta^{S}x_{ijkwz}+\beta^{T}x_{ijkwz}+\alpha^{SG}_{jk}+\epsilon_{ijkwz}$.
...
summary(step.method)
```
Due to the model being a log-linear model the explanatory variables have a multiplicative effect on the response. Exponentiating the estimated coefficients to find their effect on expenditure, first I note that the income interactions although statistically significant are not meaningful in reality as there effect on expenditure is negligible. All their coefficients have a value of approximately 1 thus have no effect on expenditure. Therefore, I ignore these interaction terms when making inference of the model as they won't change the results by any meaningful amount and it allows the effects of the exploratory variables to be generalised instead of having to consider its effect with each possible value of the variable sit has interactions with.



I consider the explantory variables in approximate importance (affects expenditure the most). 


income (with everything else constant) - interactions with  size, labour and tenure (all are insignificant) and squared term
gender - interaction with size
tenure - interaction with income 
size - interaction with gender and income
labour - interaction with income

Most important variables (can ignore the interactions with income as approximately 1). labour force (big decrease one nearly halves),size (large increase up to 80%), tenure (large increase one is 50%), sex (increase 18%- but this is then taken out by interaction with larger households)


Mention confounders, thus association not effect? As can't prove causailty
