...
Fitting the same model as in task 2 but with the log of the response expenditure (a log-linear model) $log(y_{i})=\beta_{0}+\beta_{1}x_{i}+\epsilon$ gives the below estimated coefficients and standard errors.
...
Further investigation is required for the affect of sex of household head as due to the small sub samples for some household sizes. Furthermore additional factors such as the age of the household head, ethnicity of household and location of household may improve the models accuracy.

Also I note that generalization of these results may not be possible as different countries, cultures and ethnicities may have different attitudes on spending. As there was no information on where/when the original sample was drawn from and the process for sample selection I cannot recommend what groups/ areas these results may be representative of.  
